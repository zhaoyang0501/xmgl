jQuery.adminProject = {
		...
		initSearchDataTable : function() {
			if (this.projectDataTable == null) {
				this.projectDataTable = $('#dt_table_view').dataTable({
		...
		deleteProject :function(id){
			bootbox.confirm( "是否确认删除？", function (result) {
	            if(result){
	            	$.ajax({
	        			type : "get",
	        			url : $.ace.getContextPath() + "/admin/project/delete?id="+id,
	        			dataType : "json",
		...
		showaddModal: function(id){
			$.adminProject.toSave=true;
			$("#user_modal_header_label").text("新增分类");
			$("#_modal").modal('show');
		},
		save :function (){
			if($.adminProject.toSave){
				$.ajax({
	    			type : "post",
				...
				$.ajax({
	    			type : "post",
	    			url : $.ace.getContextPath() + "/admin/project/update",
	    			data:{
	    				"project.id":$("#id").val(),
	    				"project.title":$("#title").val(),
	    				"project.context":$("#context").val()
	    			},
	    			dataType : "json",
	    			success : function(json) {
	    				if(json.resultMap.state=='success'){
		...
		showEdit: function (id){
			$("#id").val(id);
			$.adminProject.toSave=false;
			$.ajax({
    			type : "get",
    			url : $.ace.getContextPath() + "/admin/project/get?id="+id,
    			dataType : "json",
    			success : function(json) {
    				if(json.resultMap.state=='success'){
